...
while True:
    ...
    data = fileParser.fileParser("C:/Users/Jean.p/Desktop/Python Programs/temp/OnBoard-Scripts/test.csv")
    ...
    try:
        ...
        if not q.empty():  # If queue is not empty and there's connection dequeue all data to the dataArray
            for i in range(q.qsize()):
                ...
                dataArray.append(d)  # Append csv data from fileparser
            req.postData(dataArray)  # post all data
        else:
            # dataArray.append(data)  # If there's connection and the queue is empty just push data to the dataArray
            req.postData(data)  # dataArray if data is stored in dataArray
            t.sleep(3)
    except Exception:
